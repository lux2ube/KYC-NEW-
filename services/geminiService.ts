...
export const extractDataFromDocument = async (images: DocImages, docType: DocumentType): Promise<UserData> => {
  try {
    ...
    return {
      ...
      accountPurpose: '',
      specificPurpose: '',
      // FIX: Add missing properties to satisfy the UserData interface.
      occupation: '',
      sourceOfFunds: '',
    };
  } catch (e) {
...
const faceMatchSchema = {
  type: Type.OBJECT,
  properties: {
    match: { type: Type.BOOLEAN, description: "True if the faces belong to the same person, false otherwise." },
    reason: { type: Type.STRING, description: "A brief explanation for the decision, in Arabic." },
  },
  required: ["match", "reason"]
};

export const verifyFaceMatch = async (idImage: string, selfieImage: string): Promise<{match: boolean; reason: string}> => {
  try {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const model = "gemini-2.5-flash";

    const parts = [
      { text: "أنت خبير في التحقق من الهوية. قارن الوجه في صورة بطاقة الهوية مع الوجه في صورة السيلفي. هل هما لنفس الشخص؟ أجب فقط بكائن JSON بناءً على المخطط المحدد." },
      { text: "صورة بطاقة الهوية:" },
      fileToGenerativePart(idImage),
      { text: "صورة السيلفي:" },
      fileToGenerativePart(selfieImage),
    ];
    ...
    const response = await ai.models.generateContent({
        ...
        config: {
            responseMimeType: "application/json",
            responseSchema: faceMatchSchema,
        },
    });
    ...
    let jsonData;
     try {
      ...
      console.error("Failed to parse JSON from Gemini response for face match:", text, e);
      throw new Error("فشل في فهم رد التحقق من الوجه. يرجى المحاولة مرة أخرى.");
    }

    return {
        match: jsonData.match === true,
        reason: jsonData.reason || 'لم يتم تقديم سبب.'
    };
  } catch (e) {
    console.error("Error in verifyFaceMatch:", e);
    if (e instanceof Error && (e.message.includes('API key not valid') || e.message.includes('Requested entity was not found') || e.message.includes('API key is missing'))) {
        throw new Error('API_KEY_INVALID');
    }
    throw new Error("فشل التحقق من تطابق الوجه. يرجى المحاولة مرة أخرى.");
  }
};